...
END IF;
RETURN (select sum(price) FROM (deal JOIN vehicle ON deal.vehicle_fkey = vehicle.id) where dealer_fkey = dealer_id
AND DATE_PART(mode, date::date) = date_part);
END
$$ LANGUAGE plpgsql;
...
AS $$
BEGIN
RETURN dealer_revenue(dealer_id, year, 'month');
END
$$ LANGUAGE plpgsql;
...
ON repair.mechanic_fkey = vehicle_service.mehcanic_fkey) JOIN operation ON operation.id = operation_fkey)
WHERE service_fkey = service_id AND DATE_PART('year', date::date) = year);
END
$$ LANGUAGE plpgsql;
...
AS $$
BEGIN
RETURN service_revenue(service_id, year, 'month');
END
$$ LANGUAGE plpgsql;
